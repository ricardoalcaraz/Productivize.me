class HabitRepository {
  ...
  async GetAllUserHabits (userName) {
    ...
    const response = await this.pool.query(sqlQuery, [userName])
  ...
  async InsertUserHabit (habit) {
    ...
    const response = await this.pool.query(insertText, [habit.UserName, habit.HabitDescription, habit.HabitFrequency])
    return response
  }
};
